...
class Sequential2(nn.Sequential):
    ...
    def forward2(self, input1, input2):
        for module in self._modules.values():
            input1, input2 = module.forward2(input1, input2)
        return input1, input2

class ReLU2(nn.ReLU):
    def __init__(self, *args, **kwargs):
        super(ReLU2, self).__init__( 0, *args, **kwargs)

    def forward2(self, input1, input2):
        return super(ReLU2, self).forward(input1), super(ReLU2, self).forward(input2)

class Linear2(nn.Linear):
    def __init__(self, *args, **kwargs):
        super(Linear2, self).__init__(*args, **kwargs)

    def forward2(self, input1, input2):
        W_p = self.weight.clone().detach()
        W_p[W_p < 0] = 0
        W_m = self.weight.clone().detach()
        W_m[W_m > 0] = 0
        lower = F.linear(input1, W_p) + F.linear(input2, W_m, self.bias)
        upper = F.linear(input2, W_p) + F.linear(input1, W_m, self.bias)
        return lower, upper

class Conv2d2(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        super(Conv2d2, self).__init__(*args, **kwargs)
    ...
    def forward2(self, input1, input2):
        ...
        W_m = self.weight.clone().detach()
        W_m[W_m > 0] = 0
        lower = F.conv2d(input1, W_p, None, self.stride,
                        self.padding, self.dilation, self.groups) \
                + F.conv2d(input2, W_m, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        upper = F.conv2d(input2, W_p, None, self.stride,
                        self.padding, self.dilation, self.groups) \
                + F.conv2d(input1, W_m, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        return lower, upper
...
def mnist_model():
    model = Sequential2(
        Conv2d2(1, 16, 4, stride=2, padding=1),
        ReLU2(),
        Conv2d2(16, 32, 4, stride=2, padding=1),
        ReLU2(),
        Flatten(),
        Linear2(32*7*7,100),
        ReLU2(),
