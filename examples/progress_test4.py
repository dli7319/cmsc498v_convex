...
ibp_files = open(test + "/ibp_full_test.log", "r")
ibp_arr = ibp_files.readlines()
ibp_arr2 = []
for i, val in enumerate(ibp_arr):
    filteredstr = re.sub("\n", "", val)
    filteredstr = re.sub("\,\s", ",", filteredstr)
    ibp_arr2.append(filteredstr.split(" "))
    ibp_arr2[i][1] = re.match("tensor\(([\d\.]*),", ibp_arr2[i][1]).group(1)
    ibp_arr2[i][2] = re.match("tensor\(([\d\.]*),", ibp_arr2[i][2]).group(1)

# [time, mix_err, madry_err, robust_err]
ibp_arr2 = np.array(ibp_arr2, dtype=float)
...
# plt.plot(ibp_arr2[:,0], ibp_arr2[:,1], label='ibp')
#
...
# plt.plot(ibp_arr2[:,0], ibp_arr2[:,2], label='ibp')
#
# plt.xlabel('Time (seconds)')
# plt.ylabel('Error')
...
# plt.plot(ibp_arr2[:,0], ibp_arr2[:,3], label='ibp')
#
# plt.xlabel('Time (seconds)')
# plt.ylabel('Error')
...
df = df.append(pd.DataFrame(
  np.transpose([np.repeat("IBP", ibp_arr2.shape[0]),
                ibp_arr2[:, 0],
                ibp_arr2[:, 1]]), columns=["method", "x", "y"]))
...
df = df.append(pd.DataFrame(
    np.transpose([np.repeat("Baseline", baseline_arr2.shape[0]),
                  baseline_arr2[:, 0],
                  baseline_arr2[:, 2]]), columns=["method", "x", "y"]))
df = df.append(pd.DataFrame(
    np.transpose([np.repeat("Madry", madry_arr2.shape[0]),
                  madry_arr2[:, 0],
                  madry_arr2[:, 2]]), columns=["method", "x", "y"]))
df = df.append(pd.DataFrame(
    np.transpose([np.repeat("Provable", robust_arr2.shape[0]),
                  robust_arr2[:, 0],
